...
### EXPRESSION ###
##################
expression = pd.read_table('./data/GSE108497_expr.tsv')
metadata = pd.read_table('./data/GSE108497_meta.tsv')


metadata["Pathology"] = metadata.Condition.map({"Healthy":0, "SLE":1})
...
for i in range(10):
  model = xgb.XGBClassifier(objective = 'binary:logistic', tree_method = 'gpu_hist', n_jobs=4)
  ...
  test_Results_Expr[i] = {'accuracy': metr.accuracy_score(exprLabels_validation, predicted),
    ...
    'recall': metr.recall_score(exprLabels_validation, predicted, average='macro'),
    'f1': metr.f1_score(exprLabels_validation, predicted, average='macro'),
    'MCC': matthews_corrcoef(exprLabels_validation, predicted)}



with open('bestParamsGSE108497', 'wb') as bestParamsExpr_file:
  pickle.dump(bestParamsExpr, bestParamsExpr_file)

with open('test_Results_GSE108497', 'wb') as test_Results_Expr_file:
...
### METHYLATION ###
###################
methylation = pd.read_table('./data/GSE166373_meth.tsv')
metadata = pd.read_table('./data/GSE166373_meta.tsv')


metadata["Pathology"] = metadata.description.map({"Non-case":0, "Case":1})

methylation_trans = methylation.transpose() # we need samples in columns
...
with open('bestParamsGSE166373', 'wb') as bestParamsMeth_file:
  pickle.dump(bestParamsMeth, bestParamsMeth_file)

with open('test_Results_GSE166373', 'wb') as test_Results_Meth_file:
