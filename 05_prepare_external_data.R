...
##############################
biomarkersExpr <- read.delim("Results/Biomarkers_Expr.txt", header = F)
biomarkersMeth <- read.delim("Results/Biomarkers_Meth.txt", header = F)
...
GSE108497_ENSEMBL <- getBM(attributes = c('ensembl_gene_id', "hgnc_symbol"),
                           filters = 'hgnc_symbol',
...
expr_GSE108497 <- expr_GSE108497[rownames(expr_GSE108497) %in% GSE108497_ENSEMBL$hgnc_symbol,]
...
# Save the files
write.table(mvalGSE166373, "data/GSE166373_meth.tsv", sep="\t", quote = F)
write.table(metaGSE166373, "data/GSE166373_meta.tsv", sep="\t", quote = F)
